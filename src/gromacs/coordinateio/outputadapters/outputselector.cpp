...
static void adjustAtomInformation(t_atoms* atoms, t_atoms* selectionAtoms, const Selection& sel)
{
    ...
    ArrayRef<const int>     atomIndices = sel.atomIndices();
    ...
    {
        // For subset trajectories read with -fgroup, refId() indexes the
        // current frame-local position list, while atomIndices() keeps the
        // original topology atom numbering. Atom metadata must use the latter.
        const int topologyAtomIndex = atomIndices[i];
        selectionAtoms->atom[i]     = atoms->atom[topologyAtomIndex];
        selectionAtoms->atomname[i] = atoms->atomname[topologyAtomIndex];
        if (selectionAtoms->haveType)
        {
            selectionAtoms->atomtype[i] = atoms->atomtype[topologyAtomIndex];
        }
        if (selectionAtoms->haveBState)
        {
            selectionAtoms->atomtypeB[i] = atoms->atomtypeB[topologyAtomIndex];
        }
        if (selectionAtoms->havePdbInfo)
        {
            selectionAtoms->pdbinfo[i] = atoms->pdbinfo[topologyAtomIndex];
        }
    }
