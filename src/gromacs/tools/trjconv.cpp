...
static std::vector<int> buildCompressedTrajectoryIndexMap(const gmx_mtop_t& mtop, int trajectoryNatoms)
{
    ...
    std::vector<int> globalToTrajectoryIndex(mtop.natoms, -1);
    int              compressedNatoms = 0;
    for (int atomIndex = 0; atomIndex < mtop.natoms; atomIndex++)
    {
        if (getGroupType(mtop.groups, SimulationAtomGroupType::CompressedPositionOutput, atomIndex) == 0)
        {
            globalToTrajectoryIndex[atomIndex] = compressedNatoms++;
        }
    }

    if (compressedNatoms == 0 || compressedNatoms == mtop.natoms || compressedNatoms != trajectoryNatoms)
    {
        return {};
    }

    return globalToTrajectoryIndex;
}

static int* remapIndicesToCompressedTrajectory(const std::vector<int>& globalToTrajectoryIndex,
                                               const int*              globalIndices,
                                               int                     numIndices,
                                               const char*             groupPurpose,
                                               int                     trajectoryNatoms)
{
    int* remappedIndices = nullptr;
    snew(remappedIndices, numIndices);

    for (int i = 0; i < numIndices; i++)
    {
        const int globalIndex = globalIndices[i];
        if (globalIndex < 0 || globalIndex >= static_cast<int>(globalToTrajectoryIndex.size()))
        {
            gmx_fatal(FARGS,
                      "Atom index %d in the %s group is outside the topology atom range.",
                      globalIndex + 1,
                      groupPurpose);
        }

        const int trajectoryIndex = globalToTrajectoryIndex[globalIndex];
        if (trajectoryIndex < 0)
        {
            gmx_fatal(FARGS,
                      "Atom %d from the %s group is not part of the compressed-x-grps subset stored "
                      "in the trajectory (%d atoms).\n"
                      "Use a matching subset .tpr generated with gmx convert-tpr, or choose a %s "
                      "group fully contained in compressed-x-grps.",
                      globalIndex + 1,
                      groupPurpose,
                      trajectoryNatoms,
                      groupPurpose);
        ...
        remappedIndices[i] = trajectoryIndex;
    }
...
#define NPA asize(pa)
    ...
    int *        trajectoryIndex = nullptr, *trajectoryCenterIndex = nullptr;
    char*        grpnm = nullptr;
    int *        frindex, nrfri;
    char*        frname;
    int          ifit;
    ...
    gmx_bool     bRemappedCompressedTrajectoryIndices = FALSE;
...
#define NFILE asize(fnm)
    ...
    else
    {
        ...
        if (bTPS)
        {
            ...
            if (!mtop && gmx_fexist(top_file) && fn2bTPX(top_file))
            {
                int temp_natoms = -1;
                mtop            = std::make_unique<gmx_mtop_t>();
                read_tpx(top_file, nullptr, nullptr, &temp_natoms, nullptr, nullptr, mtop.get());
            }
        ...
        if (bHaveFirstFrame)
        {
            ...
            if (!bRemappedCompressedTrajectoryIndices && mtop && index != nullptr && atoms != nullptr
                && natoms != atoms->nr)
            {
                const std::vector<int> globalToTrajectoryIndex =
                        buildCompressedTrajectoryIndexMap(*mtop, natoms);
                if (!globalToTrajectoryIndex.empty())
                {
                    if (bReset || bPFit || bCluster)
                    {
                        gmx_fatal(FARGS,
                                  "This trajectory contains only the compressed-x-grps subset "
                                  "(%d atoms), but the requested trjconv operation requires atom "
                                  "indices that still match the full .tpr.\n"
                                  "Please generate a matching subset .tpr with gmx convert-tpr "
                                  "before using fit or cluster options on this trajectory.",
                                  natoms);
                    }

                    trajectoryIndex = remapIndicesToCompressedTrajectory(
                            globalToTrajectoryIndex, index, nout, "output", natoms);
                    if (bCenter && cindex != nullptr)
                    {
                        trajectoryCenterIndex = remapIndicesToCompressedTrajectory(
                                globalToTrajectoryIndex, cindex, ncent, "centering", natoms);
                    }

                    bRemappedCompressedTrajectoryIndices = TRUE;
                    fprintf(stderr,
                            "Input trajectory contains %d atoms from compressed-x-grps; "
                            "remapping selected atom indices to trajectory-local numbering.\n",
                            natoms);
                }
            }
            ...
            if (bIndex)
            {
                ...
                for (i = 0; i < nout; i++)
                {
                    ...
                    bCopy = bCopy || (i != trajectoryIndex[i]);
                }
            }
                ...
                if (bWriteFrame)
                {
                    ...
                    if (bDoIt || bTDump)
                    {
                        ...
                        if (!bPFit)
                        {
                            ...
                            if (bCenter)
                            {
                                center_x(ecenter, fr.x, fr.box, natoms, ncent, trajectoryCenterIndex);
                            }
                        }
                        ...
                        if (bCopy)
                        {
                            ...
                            for (i = 0; i < nout; i++)
                            {
                                copy_rvec(fr.x[trajectoryIndex[i]], frout.x[i]);
                                if (frout.bV)
                                {
                                    copy_rvec(fr.v[trajectoryIndex[i]], frout.v[i]);
                                }
                                if (frout.bF)
                                {
                                    copy_rvec(fr.f[trajectoryIndex[i]], frout.f[i]);
                                }
                            }
                        }
    ...
    sfree(fmem);
    sfree(grpnm);
    if (trajectoryIndex != index)
    {
        sfree(trajectoryIndex);
    }
    if (trajectoryCenterIndex != cindex)
    {
        sfree(trajectoryCenterIndex);
    }
    sfree(index);
    sfree(cindex);
    done_frame(&fr);
